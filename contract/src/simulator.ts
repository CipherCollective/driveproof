...
export type AttestorConfig = {
  id?: bigint;
  /** Omitted when trips are signed externally (e.g. attestor HTTP). */
  secretKey?: bigint;
...
export class DriveProofSimulator {
  ...
  constructor(
    ...
    if (attestor) {
      this.attestorSk = attestor.secretKey ?? 0n;
      this.attestorPk = attestor.publicKey;
      this.attestorId = attestor.id ?? 1n;
    } else {
